...
В качестве платформы для протокола Стикеля я выбрал группу матриц над кольцом усеченных многочленов над $ZZ_n$. Усеченные многочлены --- это выражения вида $sum_(k = 0)^N a_k x^k$ с обычным сложением и умножением по правилу $x^i dot x^j = x^(i + j mod(N + 1))$, то есть это фактор-кольцо $faktor(ZZ_(n)[x], (x^(N + 1)))$. Идеал порожденный  многочленом $x^(N + 1)$ довольно прост, вычисления в этом фактор-кольце довольно эффективны. Кроме того, большое ключевое пространство обеспечивается с низкими затратами; например, при $n = 100, N = 20$ существует $100^21 = 10^42$ усеченных многочленов, что дает $10^168$ матриц размера $2 times 2$ над этим кольцом.
